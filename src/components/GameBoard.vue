<template>
  <div class="game-board">
    ...
    <div
      v-for="hazard in hazards" :key="hazard.id"
      class="hazard" :class="{ 'hazard-fast': hazard.fast }"
      :style="{ left: hazard.x + 'px', top: hazard.y + 'px' }"
    >💀</div>

    <div
      v-for="bullet in bullets" :key="bullet.id"
      class="bullet"
...
<script setup>
defineProps({ player: Object, hazards: Array, bullets: Array, gems: Array })
</script>

<style scoped lang="scss">
.game-board {
  width: 800px; height: 500px; background: #0d1117;
...
.hazard-fast {
  ...
  animation: float-fast 0.3s ease-in-out infinite alternate;
}
@keyframes float {
...
@keyframes float-fast {
  ...
  to   { transform: translate(-50%, -52%) scale(1.2); }
}
.bullet {
  width: 8px; height: 8px; border-radius: 50%;
  background: #fac775;
  box-shadow: 0 0 6px #fac775, 0 0 12px rgba(250,200,100,0.5);
...
.gem {
  ...
  animation: gemBob 1s ease-in-out infinite alternate;
}
@keyframes gemBob {
  from { transform: translate(-50%, -50%) scale(1); }
